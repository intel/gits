...
void AddressPinningUseLayer::readAddressRanges() {
  ...
  while (inputFile >> key >> range.StartAddress >> range.SizeInBytes) {
    addressRangesByKey_[key] = range;
...
void AddressPinningUseLayer::preResource(CommandT& command) {
  ...
  auto it = addressRangesByKey_.find(command.ppvResource_.key);
  if (it == addressRangesByKey_.end()) {
    return;
  }
...
void AddressPinningUseLayer::preHeap(CommandT& command) {
  ...
  auto it = addressRangesByKey_.find(command.ppvHeap_.key);
  if (it == addressRangesByKey_.end()) {
    return;
  }
  deviceTools_->SetNextAllocationAddress(it->second.StartAddress);
...
void AddressPinningUseLayer::postHeap(CommandT& command) {
  ...
  GITS_ASSERT(hr == S_OK);
  GITS_ASSERT(it->second.StartAddress == range.StartAddress);
}
...
void AddressPinningUseLayer::preOpenExistingHeap(CommandT& command) {
  ...
  heapDesc.SizeInBytes = it->second.SizeInBytes;
