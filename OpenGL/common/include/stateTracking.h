...
inline void glBufferStorage_SD(
  ...
  flags = flags | Configurator::Get().opengl.recorder.bufferStorageFlagsMask;
  if (size < 0 || target == 0) {
    return;
  }
...
inline void glBindTexture_SD(GLenum target, GLuint texture, GLboolean recording = 0) {
  ...
  bool isCaptureTex = !Configurator::Get().opengl.player.capture2DTexs.empty() ||
                      !Configurator::Get().opengl.player.captureDraws2DTexs.empty();
  if (Configurator::IsRecorder() || (Configurator::IsPlayer() && curctx::IsEs() && isCaptureTex)) {
...
inline void glTexStorage1D_SD(
  ...
  if (!isTargetProxy(target) && (Configurator::IsRecorder() ||
                                 (Configurator::IsPlayer() && curctx::IsEs() && isCaptureTex))) {
...
inline void glTexImage2D_SD(GLenum target,
  ...
  if (!isTargetProxy(target) && ((Configurator::IsRecorder() && currentContext != 0) ||
                                 (Configurator::IsPlayer() && curctx::IsEs() && isCaptureTex))) {
    TextureStateObject(target).SetTexLevelParams(level, width, height, 1, internalformat, border, 0,
                                                 0, 1, 0, format, type);
  }
...
inline void glCompressedTexImage2D_SD(GLenum target,
  ...
  // resources
  if (Configurator::IsRecorder() && !curctx::IsOgl() && recording &&
...
inline void glMapBufferRange_SD(GLvoid* return_value,
                                ...
                                GLbitfield access,
                                GLboolean recording = 0) {
  access = access & Configurator::Get().opengl.recorder.bufferMapAccessMask;
  bool error = false;
  GLint buffer = boundBuff(target);
  if (buffer > 0) {
    CBufferStateObj* bufferState = SD().GetCurrentSharedStateData().Buffers().Get(buffer);
...
inline void glCompressedTextureSubImage2D_SD(GLuint texture,
  ...
  if (!curctx::IsOgl()) {
    ...
    const auto& texState = Configurator::Get().opengl.recorder.texturesState;
