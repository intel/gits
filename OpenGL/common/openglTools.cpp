...
namespace {
auto genQueries = &drv.gl.glGenQueries; // At this point pointers are not yet valid
auto beginQuery = &drv.gl.glBeginQuery; // here only for type deduction.

auto endQuery = &drv.gl.glEndQuery;
auto deleteQueries = &drv.gl.glDeleteQueries;
auto getQueryObjectuiv = &drv.gl.glGetQueryObjectuiv;
auto queryCounter = &drv.gl.glQueryCounter;

void InitializeQueryAPIs() {
  static bool init = false;
  if (!init) {
    genQueries = &drv.gl.glGenQueries;
    beginQuery = &drv.gl.glBeginQuery;
    endQuery = &drv.gl.glEndQuery;
    deleteQueries = &drv.gl.glDeleteQueries;
    getQueryObjectuiv = &drv.gl.glGetQueryObjectuiv;

    // ES apis will use EXT versions of functions if avaiable.
    if (!curctx::IsOgl() && drv.egl.eglGetProcAddress("glGenQueriesEXT") != nullptr) {
      genQueries = &drv.gl.glGenQueriesEXT;
      beginQuery = &drv.gl.glBeginQueryEXT;
      endQuery = &drv.gl.glEndQueryEXT;
      deleteQueries = &drv.gl.glDeleteQueriesEXT;
      getQueryObjectuiv = &drv.gl.glGetQueryObjectuivEXT;
      queryCounter = &drv.gl.glQueryCounterEXT;
    }

    if (!curctx::IsOgl() && drv.egl.eglGetProcAddress("glQueryCounterINTEL") != nullptr &&
        drv.egl.eglGetProcAddress("glQueryCounterEXT") == nullptr) {
      queryCounter = &drv.gl.glQueryCounterINTEL;
    }
    init = true;
  }
}
...
void GLQueryWrapper::Begin(GLuint drawnr, bool clearblit) {
  InitializeQueryAPIs();
  ...
  _queries.back().clearOrBlit = clearblit;
}
void GLQueryWrapper::End() {
  (*endQuery)(_queryType);
  _queries.back().ended = true;
}
void GLQueryWrapper::GetResults() {
  for (auto& query : _queries) {
    if (!query.ended || query.result != -1) {
      continue;
    }
    query.result = 0;
    (*getQueryObjectuiv)(query.queryObj, GL_QUERY_RESULT, (GLuint*)&query.result);
    (*deleteQueries)(1, &query.queryObj);
  }
}
