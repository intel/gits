...
public:
  ...
  void* getImpl() override {
    if (!m_PluginLayer) {
      ...
      cfg.WarmupFrames = cfgYaml["Config"]["WarmupFrames"].as<unsigned>();
...
std::unique_ptr<gits::vulkan::BenchmarkPlugin> g_Plugin;
...
GITS_PLUGIN_API IPlugin* createPlugin(IPluginContext context, const char* pluginPath) {
  ...
  if (!g_Plugin) {
    g_Plugin = std::make_unique<gits::vulkan::BenchmarkPlugin>(context, pluginPath);
  }
  return g_Plugin.get();
}
