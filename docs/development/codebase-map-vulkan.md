...
| `analyzerLayerAuto.{h,cpp}` | `AnalyzerLayer : Layer` — adds every handle touched by a command to the restore set via `AnalyzerService::AddObjectForRestore`/`AddObjectsForRestore`, feeding the analysis pass below. |
