...
- Open the copied `gits_config.txt` and in the `Extras -> Utilities`
  ...
  - `ExtendedDiagnostic` to `False` . This is to avoid a hang at the start.

  - `RemoveAPISharing` to `"DX"` . This will get rid of DX calls and allow us to create a OCL/OGL-only stream.
