...
CGits::~CGits() {
  try {
    ...
    if ((Configurator::IsRecorder() && Configurator::Get().common.recorder.enabled)
        ...
        || (Configurator::IsPlayer() && Configurator::Get().directx.features.subcapture.enabled)
#endif
    ) {
      CGits::Instance().GetMessageBus().publish(
          {PUBLISHER_RECORDER, TOPIC_STREAM_SAVED},
          std::make_shared<StreamSavedMessage>(
              Configurator::Get().common.recorder.dumpPath.string()));
    }

    // Create a buildable CCode project by automatically copying necessary files.
    // TODO: extract it to a separate function.
    if (Configurator::IsRecorder() && Configurator::DumpCCode()) {
      auto& dumpPath = Configurator::Get().common.recorder.dumpPath;
      auto& installPath = Configurator::Get().common.recorder.installPath;
...
zipFile CGits::OpenZipFileGLPrograms() {
  ...
  std::filesystem::path path = Configurator::Get().common.recorder.dumpPath / "gitsPrograms.zip";
...
void CGits::OpenUnZipFileGLPrograms() {
  ...
  std::filesystem::path arch_path =
      Configurator::Get().common.player.streamDir / "gitsPrograms.zip";
...
void CFile::SetDiagnosticInfo(std::filesystem::path configPath) {
  gits::gather_diagnostic_info(*_properties);
...
void CGits::ResourceManagerInit(const std::filesystem::path& dump_dir) {
  ...
  if (Configurator::IsPlayer() && stream_older_than(GITS_TOKEN_COMPRESSION)) {
...
bool stream_older_than(uint64_t version) {
  return Configurator::IsPlayer() && (CGits::Instance().FilePlayer().Version().version() < version);
}
