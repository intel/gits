...
gits::CRecorder::CRecorder()
  ...
  if (config.common.recorder.enabled ||
      (Configurator::IsPlayer() && config.common.player.subcapture.enabled)) {
#else
  if (config.common.recorder.enabled) {
#endif
...
void gits::CRecorder::Register(std::unique_ptr<CBehavior> behavior) {
  ...
  bool recEnabled = rec.enabled || (Configurator::IsPlayer() && cmm.player.subcapture.enabled);
#else
  ...
  if (Configurator::IsPlayer() && cmm.player.subcapture.enabled) {
  ...
  if ((Configurator::IsRecorder() && rec.enabled) ||
      (Configurator::IsPlayer() && cmm.player.subcapture.enabled)) {
#else
  if (Configurator::IsRecorder() && rec.enabled) {
#endif
    // create file
...
void gits::CRecorder::Save() {
  ...
  bool recEnabled = cfg.common.recorder.enabled ||
                    (Configurator::IsPlayer() && cfg.common.player.subcapture.enabled);
#else
  bool recEnabled = cfg.common.recorder.enabled;
#endif
  if (!recEnabled) {
