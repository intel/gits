...
void gather_diagnostic_info(nlohmann::ordered_json& properties) {
  ...
  if (Configurator::Get().common.recorder.extendedDiagnosticInfo) {
    properties["diag"]["gits"]["uname"] =
        CommandOutput("uname -a", Configurator::Get().common.mode == GITSMode::MODE_RECORDER);
    properties["diag"]["gits"]["lspci"] =
        CommandOutput("lspci -v", Configurator::Get().common.mode == GITSMode::MODE_RECORDER);
    properties["diag"]["gits"]["maps"] =
        CommandOutput("cat /proc/" + my_pid + "/maps",
                      Configurator::Get().common.mode == GITSMode::MODE_RECORDER);
    properties["diag"]["gits"]["cpuinfo"] = CommandOutput(
        "cat /proc/cpuinfo", Configurator::Get().common.mode == GITSMode::MODE_RECORDER);
  }
