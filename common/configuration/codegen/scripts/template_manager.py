...
class TemplateManager:
    ...
    def render_task(self, task, context):
        print(f"  [{task.step}] - {len(self.templates[task.step])} file(s) to write")

        context['time'] = datetime.now()
        for i, (templateName, output_file_path, meta_data) in enumerate(self.templates[task.step]):
            ...
            print(
                f"  {i + 1}/{len(self.templates[task.step])} - {templateName} -> {os.path.normpath(output_file)}")
