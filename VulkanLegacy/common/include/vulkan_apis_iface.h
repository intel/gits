...
public:
  ...
  virtual bool CfgRec_IsAllMode() const {
    return Configurator::Get().vulkan.recorder.mode == TVulkanRecorderMode::ALL;
  }
  virtual bool CfgRec_IsFramesMode() const {
    return Configurator::Get().vulkan.recorder.mode == TVulkanRecorderMode::FRAMES;
  }
  virtual unsigned CfgRec_ExitFrame() const {
    return Configurator::Get().vulkan.recorder.all.exitFrame;
  }
  virtual int CfgRec_StartFrame() const {
    return Configurator::Get().vulkan.recorder.frames.startFrame;
  }
  virtual int CfgRec_StopFrame() const {
    return Configurator::Get().vulkan.recorder.frames.stopFrame;
  }
  virtual const std::vector<unsigned>& CfgRec_StartKeys() const {
    return Configurator::Get().vulkan.recorder.frames.startKeys;
  }
  ...
  virtual bool CfgRec_IsDrawsRangeMode() const {
    return Configurator::Get().vulkan.recorder.objRange.rangeSpecial.objMode ==
  ...
  virtual bool CfgRec_IsSubcapture() const {
    return !CfgRec_IsAllMode() && Configurator::IsRecorder();
  }
