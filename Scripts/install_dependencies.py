...
class Boost(Repository):
    ...
    def build(self):
        ...
        self.execute(
            f"./b2 address-model={self.address_model} install cxxflags=-fPIC runtime-link=static --with-container --with-filesystem --with-regex --with-thread --prefix={str(THIRD_PARTY_PATH / self.name)} -j8 -d0 --hash",
            cwd=boost_dir,
        )
