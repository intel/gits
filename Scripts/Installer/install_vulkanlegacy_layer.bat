...
set layer_json_name=VkLayer_vulkan_GITS_recorder_legacy.json
...
(for /f "usebackq" %%a in (`reg query "%registry_path_32%" /S ^| find "%layer_json_name%"`) do (
  ...
  echo The previous 32bit GITS VulkanLegacy recorder layer has been removed from the registry
)) 2> NUL

reg add "%registry_path_32%" /v %~f2%layer_json_name% /t REG_DWORD /d 0
if %errorlevel%==0 (
  echo 32bit GITS VulkanLegacy recorder layer has been added to the registry
) else (
...
(for /f "usebackq" %%a in (`reg query "%registry_path%" /S ^| find "%layer_json_name%"`) do (
  ...
  echo The previous GITS VulkanLegacy recorder layer has been removed from the registry
)) 2> NUL

reg add "%registry_path%" /v %~f2%layer_json_name% /t REG_DWORD /d 0
if %errorlevel%==0 (
  echo GITS VulkanLegacy recorder layer has been added to the registry
) else (
  echo Couldn't add layer to registry
)
goto:EOF
