...
CapturePlayerShaderIdentifierService& CapturePlayerShaderIdentifierService::Get() {
  static CapturePlayerShaderIdentifierService instance;
  return instance;
}

void CapturePlayerShaderIdentifierService::AddCaptureShaderIdentifier(
    unsigned commandKey, ShaderIdentifier captureIdentifier, LPCWSTR exportName) {
...
void CapturePlayerShaderIdentifierService::AddPlayerShaderIdentifier(
    unsigned commandKey, ShaderIdentifier playerIdentifier, LPCWSTR exportName) {
  std::lock_guard<std::mutex> lock(m_Mutex);

  auto itByCommandKey = m_ShaderIdentifiersByCommandKey.find(commandKey);
  assert(itByCommandKey != m_ShaderIdentifiersByCommandKey.end());
...
CapturePlayerShaderIdentifierService::ShaderIdentifier* CapturePlayerShaderIdentifierService::
