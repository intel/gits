...
void DescriptorSetUpdateService::RestoreUpdates(uint64_t setKey,
      ...
      if (IsImageDescriptorType(ed.DescriptorType)) {
        if (ed.DescriptorType == VK_DESCRIPTOR_TYPE_SAMPLER ||
          ...
          if (ed.SamplerKey != 0 && !sts.IsRestored(ed.SamplerKey)) {
            LOG_TRACE << "Vulkan subcapture: omitting descriptor write for set key=" << setKey
  ...
  if (!writes.empty()) {
    ...
    if (!dsState) {
      LOG_TRACE << "Vulkan subcapture: skipping descriptor update restore for set key=" << setKey
